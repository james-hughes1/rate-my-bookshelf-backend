...
def draw_segment_boundaries(
    image, all_masks, boundary_color=(255, 165, 0), thickness=2
):
    ...
    for contour in contours:
        ...
        for i in range(len(contour)):
            ...
            for offset in range(0, seg_len, dash_length * 2):
                ...
                s = (
                    int(p1[0] + (p2[0] - p1[0]) * start_t),
                    int(p1[1] + (p2[1] - p1[1]) * start_t),
                )
                e = (
                    int(p1[0] + (p2[0] - p1[0]) * end_t),
...
def create_segmentation_gif(
    ...
    for depth in range(max_depth + 1):
        if len(depth_masks[depth]) > 0:
            ...
            frame = draw_segment_boundaries(
                image, all_masks_so_far, boundary_color, thickness
            )
